...
do
  ...
  while read -r LINE; do
    fastq-dump -O $G/C_gig_He_OsHV1_SRA/ --readids $LINE
  done < $f
done
...
do
  prefetch --option-file $f
  while read -r LINE; do
    fastq-dump -O $G/C_gig_Zhang_Vibrio_SRA/ --readids $LINE
  done < $f
done
...
do
  prefetch --option-file $f
  while read -r LINE; do
    fastq-dump -O $C/C_vir_ROD_SRA/ --readids $LINE
  done < $f
done
...
do
  prefetch --option-file $f
  while read -r LINE; do
    fastq-dump -O $G/C_gig_deLorgeril_OsHV1_SRA/ --split-files --readids $LINE
  done < $f
done
...
do
  prefetch --option-file $f
  while read -r LINE; do
    fastq-dump -O $G/C_gig_Rubio_Vibrio_SRA/ --split-files --readids $LINE
  done < $f
done
...
do
  prefetch --option-file $f
  while read -r LINE; do
    fastq-dump -O $C/C_vir_Probiotic_SRA/ --split-files --readids $LINE
  done < $f
done
