...
#SBATCH --nodes=1
#SBATCH --export=NONE
#SBATCH	-o /data3/marine_diseases_lab/erin/2017_2020_Transcriptome_Analysis/pipeline_files/2020_Scripts/Script_out_error_files/Stringtie_ROD_Dermo_HE_Zhang_out_2_7_2020
#SBATCH	-e /data3/marine_diseases_lab/erin/2017_2020_Transcriptome_Analysis/pipeline_files/2020_Scripts/Script_out_error_files/Stringtie_ROD_Dermo_HE_Zhang_error_2_7_2020
...
stringtie --merge -A -G $CG/GCF_000297895.1_oyster_v9_genomic.gff -o $GLO/deLorgeril_OsHV1_stringtie_merged.gtf $GLO/deLorgeril_mergelist.txt
...
for i in ${array1[@]}; do
		stringtie -e -G $GLO/deLorgeril_OsHV1_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CG/GCF_000297895.1_oyster_v9_genomic.gff -o $GRV/Rubio_Vibrio_stringtie_merged.gtf $GRV/Rubio_Vibrio_mergelist.txt
...
#Re-estimate transcript abundance after merge step
for i in ${array2[@]}; do
		stringtie -e -G $GRV/Rubio_Vibrio_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CV/ref_C_virginica-3.0_top_level.gff3 -o $CP/Probiotic_stringtie_merged.gtf $CP/Probiotic_mergelist.txt
...
#Re-estimate transcript abundance after merge step
for i in ${array3[@]}; do
		stringtie -e -G $CP/Probiotic_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CV/ref_C_virginica-3.0_top_level.gff3 -o $CD/Dermo_stringtie_merged.gtf $CD/Dermo_mergelist.txt
echo "Dermo merged"
#gffcompare to compare how transcripts compare to reference annotation
...
#Re-estimate transcript abundance after merge step
for i in ${array4[@]}; do
		stringtie -e -G $CD/Dermo_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CG/GCF_000297895.1_oyster_v9_genomic.gff -o $GHO/He_OsHV1_stringtie_merged.gtf $GHO/He_OsHV1_mergelist.txt
...
echo "He OsHV1gffcompared"
#Re-estimate transcript abundance after merge step
for i in ${array5[@]}; do
		stringtie -e -G $GHO/He_OsHV1_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CG/GCF_000297895.1_oyster_v9_genomic.gff -o $GZV/Zhang_Vibrio_stringtie_merged.gtf $GZV/Zhang_Vibrio_mergelist.txt
...
#Re-estimate transcript abundance after merge step
for i in ${array6[@]}; do
		stringtie -e -G $GZV/Zhang_Vibrio_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
...
#Run StringTie merge, merge transcripts from all samples in single experiment
stringtie --merge -A -G $CV/ref_C_virginica-3.0_top_level.gff3 -o $CR/ROD_stringtie_merged.gtf $CR/ROD_mergelist.txt
echo "ROD merged"
#gffcompare to compare how transcripts compare to reference annotation
...
#Re-estimate transcript abundance after merge step
for i in ${array7[@]}; do
		stringtie -e -G $CR/ROD_stringtie_merged.gtf -o $(echo ${i}|sed "s/\..*//").merge.gtf ${i}
