...
mcols(resoshv1_05, use.names = TRUE)
...
hist(resoshv1_05$pvalue, breaks = 20, col = "grey") #hill

#remove filtered out genes by independent filtering, they have NA adj. pvals
resoshv1_05_df <- resoshv1_05[ !is.na(resoshv1_05$padj), ]
...
#histogram of P- values to visualize any "hills" or "U shape"
# hill means variance of the null distribution too high, U shape means variance assumed too low
hist(resBac_05$pvalue, breaks = 20, col = "grey") #hill

#remove filtered out genes by independent filtering, they have NA adj. pvals
resBac_05_df <- resBac_05[ !is.na(resBac_05$padj), ]

#remove genes with NA pvals (outliers)
resBac_05_df <- resBac_05_df[ !is.na(resBac_05_df$pvalue), ]

#remove adjsuted pvalues, since we add the fdrtool results later on (based on the correct p-values)
resBac_05_df <- resBac_05_df[, -which(names(resBac_05_df) == "padj")]

#use z-scores as input to FDRtool to re-estimate the p-value
FDR.resBac_05_df <- fdrtool(resBac_05_df$stat, statistic= "normal", plot = T)

#add values to the results data frame, also ad new BH- adjusted p-values
resBac_05_df[,"padj"] <- p.adjust(FDR.resBac_05_df$pval, method = "BH")

#replot corrected p-values
hist(FDR.resBac_05_df$pval, col = "royalblue4",
     main = "Correct null model full Bacterial Challenge Set", xlab = "CORRECTED p-values")

#Check how many genes have BH adjusted p values of less than 0.05 after P-value correction?
sum( resBac_05_df$padj < 0.05, na.rm=TRUE ) #154 (before p-value correction it was 196)

#Subset the results table to the differentially expressed genes under FDR 0.01, order the Log2FC table first by strongest down regulation
resBac_05_dfSig <- resBac_05_df[ which(resBac_05_df$padj < 0.05 ), ]
head( resBac_05_dfSig[ order( resBac_05_dfSig$log2FoldChange ), ] ) #head for strongest downregulation
tail( resBac_05_dfSig[ order( resBac_05_dfSig$log2FoldChange ), ] ) #tail for strongest up regulation

####Visualize Results with Diagnostic Plots####
#MA plot, useful overview for experiment with two-group comparison. Plots log2FC over mean of normalized counts
#genes with adjusted p value 
plotMA(resBac_05_df)
plotMA(resBac_05_dfSig)

#Export Results to CSV
write.csv( as.data.frame(resBac_05_df), file="resBac_05_df.csv")
write.csv( as.data.frame(resBac_05_dfSig), file="resBac_05_dfSig.csv")
